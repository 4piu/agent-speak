...
Media safety bounds are implementation constants rather than policy knobs: files are limited to 50 MiB and five minutes of decoded audio in the prototype. These limits are advertised by `get_audio_capabilities` but do not appear in configuration files.
...
All media bytes are untrusted, including files named by presets. The server limits file size and decoded duration, sniffs content, preflights the decoder, caps runtime playback, uses only the enabled Rust decoder implementations, and never invokes a shell, external media player, URL, or file association. Malformed-media and dependency-audit tests reduce exposure to parser bugs.
