...
- Compressed or containerized incremental provider audio.
...
- one Rodio output path used only for provider-generated speech, supporting
  complete prepared audio and incremental PCM sample queues;
...
4. construct `PreparedAudio::from_memory`, preserving decoder preflight;
5. call worker-owned `RodioAudio::play_to` with the original target/gain;
...
2. validate `synthesis.audio_begin`, including raw PCM16 format, sample rate,
   and channel count;
3. validate each frame's alignment and cumulative byte/duration limits, convert
   samples for Rodio, and append them to a bounded queue in arrival order;
...
5. keep at most two seconds of decoded samples queued; normal pipe backpressure
   slows a faster provider without growing host memory;
...
- incremental PCM alignment, cumulative bounds, prebuffer threshold, terminal
  accounting, and partial failure before device playback;
